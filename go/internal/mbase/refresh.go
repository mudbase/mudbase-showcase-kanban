...
// and persists it wherever the caller keeps session state (this app's encrypted session cookie -
// see internal/server/middleware.go's sessionMiddleware). It returns the new access token, or an
// error if refreshing itself failed (in which case the original 401 is surfaced to the caller
...
// web/src/lib/mudbase.ts) - the bug class every sibling port of this showcase had to find and fix
// was capturing a refresh token at auth time and then never actually using it on a 401. Wiring
// TokenRefresher through request context (below) and into every mbase.List/Get/Create/Update/
// Delete call (see data.go's callWithRefresh) is what makes the fix structural instead of
...
// of surfacing a raw 401 up to the visitor - mirrors the reference web app's MudbaseClient
...
// including a refresh failure itself, or a 403 (which no refresh could ever fix - see
// errors.go's IsForbidden) - is returned as-is so the original, more specific error reaches the
// caller.
