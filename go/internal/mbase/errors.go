...
// IsForbidden reports whether err represents an HTTP 403 response - Mudbase's own collection
// permissions rejecting a role that lacks the required action (e.g. a `viewer` attempting a
// write). This is the real, server-side RBAC enforcement boundary this app's own role checks
// (internal/rbac) are only a UX mirror of - see plan/build-plan.md "RBAC Matrix".
func IsForbidden(err error) bool {
	...
	var apiErr *mudbase.GenericOpenAPIError
	if errors.As(err, &apiErr) {
		return strings.HasPrefix(apiErr.Error(), "403")
	}
	return false
}
