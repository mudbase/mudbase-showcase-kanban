...
import dev.mudbase.showcase.kanban.config.MudbaseClientFactory;
...
/**
 * Auth calls against a provisioned Mudbase project: login, logout, and refresh. No registration
 * here and no anonymous/guest session - see plan/build-plan.md "Auth Model" for why this app has
 * no public read at all (every one of the three roles must sign in).
 *
 ...
 * Multi-Role feature - i.e. all three owner/member/viewer accounts - gets a login response whose
 * {@code user} object carries both {@code role} and {@code customRole}; the generated model only
 * declares {@code role}. This is the exact same bug the sibling {@code
 * mudbase-showcase-ecommerce/java} and {@code mudbase-showcase-social/java} ports already found
 * and fixed against this same backend - reused here rather than rediscovered, per the task's
 * explicit instruction. Fixed the same way: a raw call over the SDK's own shared {@code
...
public class MudbaseAuthClient {
  ...
  public AuthResult login(String email, String password) {
    ...
    try (Response response = httpClient.newCall(request).execute()) {
      ...
      return new AuthResult(
          ...
          // class javadoc. customRole is the project app-role this app cares about (owner/
          // member/viewer); fall back to role only for an account shape that predates it.
  ...
  public void logout(String bearerToken) {
    ...
    try {
      ...
      // Best-effort revoke: a failed server-side revoke must never block the user from being
      // signed out locally.
    }
  }
   ...
   * dev.mudbase.showcase.kanban.auth.SessionAuthService#recoverFromUnauthorized} to transparently
