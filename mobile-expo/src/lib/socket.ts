 ...
 * `subscribe:collection` / `db:create` / `db:update` / `db:delete` Socket.IO
 * contract, confirmed live against this exact project in
 * web/plan/build-plan.md's "Live smoke test results". `socket.io-client` works
 * unmodified in React Native: RN provides the `WebSocket` and
 * `XMLHttpRequest` globals the "websocket" and "polling" transports need, so
 * no RN-specific transport config is required.
 */
