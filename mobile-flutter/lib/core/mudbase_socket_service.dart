...
/// room (`subscribe:collection` -> `db:create`/`db:update`/`db:delete`).
///
/// This app needs `db:delete` in addition to `db:create`/`db:update` (unlike
/// the social showcase's feed) because lists and cards are actually deleted
/// here - see `BoardController`.
class MudbaseSocketService {
  ...
  /// connection) authenticated as [token]. A same-session token rotation
  /// (refresh) would otherwise leave the socket authenticated as a stale
  /// identity if this only checked "already connected" - same rationale as
  /// the web app's `connect()`.
  ...
  /// app listens to (`db:create`/`db:update`/`db:delete`) sends a JSON
  /// object shaped `{ projectId, collectionId, action, data, timestamp }`.
