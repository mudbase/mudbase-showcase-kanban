...
Unlike the sibling social/ecommerce Python ports, this app has **no
anonymous/guest session** (see plan/build-plan.md "Auth Model") — every one
of the three roles (owner/member/viewer) must present a real session, so
there is no `ensure_anonymous_session` here. `require_session` below is this
app's equivalent of the reference web app's `<AuthGate>`.

Refresh strategy (proactive margin-based refresh in `get_valid_access_token`,
reactive retry-once in `call_with_reauth`) is ported verbatim from the
sibling ports' `app/session.py`.
"""
...
from app.mudbase_client import MudbaseApiError, refresh_sync
...
async def call_with_reauth(
    ...
    up succeeding. Any non-401 error, or a 401 that persists after a
    successful refresh, propagates unchanged."""
    try:
...
async def require_session(request: Request) -> tuple[SessionUser, str] | None:
    """This app's equivalent of the reference web app's `<AuthGate>`: every
    page except /login requires a real session (there is no anonymous/guest
    read — see module docstring). Returns `(user, token)` on success, or
    `None` if the visitor must be redirected to /login (no session, or a
    refresh attempt failed and the session was cleared)."""
