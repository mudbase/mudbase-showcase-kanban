...
/// every authenticated request in the app — every collection read/write in `ListsService`,
/// `CardsService`, and `ActivityService` (via `CollectionsGateway`) routes through `perform(_:)`
/// for exactly this reason, since every role in this app (including `viewer`) requires a real,
/// signed-in session with no anonymous fallback (see `SessionStore` doc comments).
///
...
/// calls each hit a 401 at nearly the same moment (e.g. the board's poll loop and a card edit
...
/// sibling Swift ports' `Networking/AccessTokenCoordinator.swift`.
