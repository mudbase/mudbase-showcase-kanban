...
/// carry it automatically. Ported verbatim from the sibling Swift ports'
...
extension JSONValue {
    ...
    /// bodies (`update`) don't overwrite untouched fields with explicit nulls. A key whose value
    /// is present but itself `.null` IS included — that's how `CardsService.update` sends a real
    /// JSON `null` to clear `assigneeId`/`assigneeName` (see that file's doc comment).
